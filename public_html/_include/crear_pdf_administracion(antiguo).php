...
$consulta_documentos = sprintf("SELECT * FROM %s WHERE  transaccion_id= '%s'",_TBL_PQR_DOCUMENTO,$dato4->fields['transaccion_id']);
...
$pdf->SetFont('Arial','',8);
$pdf->SetTextColor('000','000','000');
$pdf->Cell(40,5,$dato4->fields['tipo_plazo_total']);//** RESUMEN
$pdf->ln(7);
...
if($dato_doc)
{
	...
	$variable2 = $dato_doc->GetRows();
			
}
foreach ($variable2 as $doc )
{
		...
		$pdf->Cell(45,5,$doc['documento_nombre'],0,0,'',0,$ruta_archivo_adjunto);//** ARCHIVO ADJUNTO
		$pdf->SetFont('Arial','B',8);
		$pdf->SetTextColor('0','0','0');
