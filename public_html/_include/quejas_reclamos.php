<?
   ...
   if (!$nombre_form) {  $error[] = _ROT_QUEJAS_NOMBRE_ERROR;  }
   if (!$apellido_form) {  $error[] = _ROT_QUEJAS_APELLIDO_ERROR;  }
   if (!$identificacion_form || !is_numeric($identificacion_form)) {  $error[] = _ROT_QUEJAS_IDENTIFICACION_ERROR;  }
   if (!$correo_form)  {  $error[] = _ROT_QUEJAS_CORREO_ERROR1;  }
   if ($correo_form && !isEmail($correo_form)) {  $error[] = _ROT_QUEJAS_CORREO_ERROR2;  }
   if (!$queja_form){  $error[] = _ROT_QUEJAS_LABEL_ERROR;  }
   ...
   if ($_SERVER['REQUEST_METHOD']=="POST" && isset($total_errores)){
	...
	if (isset($total_errores)) {
	   ...
	   $smarty->assign('c_formulario_titulo', _ROT_QUEJAS);
	
	   $campo['nombre'] = _ROT_QUEJAS_NOMBRE;
	   ...
	   $campo['nombre'] = _ROT_QUEJAS_APELLIDO;
	   $campo['clase']  = "requerido";
	   ...
	   $campo['nombre'] = _ROT_QUEJAS_IDENTIFICACION;
	   $campo['clase']  = "requerido";
	   ...
	   return $smarty->fetch('tpl_contacto.html');
        ...
        if (microsmail(_EMAIL_QUEJA_RECLAMO,sprintf("Queja o reclamo desde la página %s",_NOMBRESITIO),$body,$from,'queja_reclamos.php','1') 
        	&& microsmail("usuario@mindefensa.gov.co",sprintf("Queja o reclamo desde la página %s",_NOMBRESITIO),$body,$from,'queja_reclamos.php','1') 
        	&& microsmail("faoip.webmaster@fac.mil.co",sprintf("Queja o reclamo desde la página %s",_NOMBRESITIO),$body,$from,'queja_reclamos.php','1') 
        	&& microsmail($correo_form,strip_tags(_NOMBRESITIO),$body2,"From: "._EMAIL,'queja_reclamos.php','2') ){
		
          $smarty1 = new Smarty_Plantilla;
          $smarty1->assign('rotMensaje',_ROT_CONFIRMACION);
          $smarty1->assign('tipo'      ,"exito");
          $error[] = sprintf(_ROT_QUEJAS_ENVIO_CONFIRMA,_EMAIL);
          $smarty1->assign('elementoMenu',$error);
          ...
          $smarty1->assign('rotMensaje',_ROT_ADVERTENCIA);
          $smarty1->assign('tipo'      ,"error");
          $error[] = sprintf(_ROT_QUEJAS_ENVIO_ERROR,_EMAIL);
          $smarty1->assign('elementoMenu',$error);
          return $smarty1->fetch('tpl_display_mensaje.html');
