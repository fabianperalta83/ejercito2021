<?
   ...
   $campo['clase']  = "requerido";
   $campo['input']  = sprintf("<div class='form-group'><input type=text name=nombre_form size=35 value='%s' class='form-control tpl_input_edicion_requerido'></div>",$nombre_form);
   $campos[] = $campo;

   $campo['nombre'] = _ROT_CONTACTO_EMAIL;
   $campo['clase']  = "requerido";
   $campo['input']  = sprintf("<div class='form-group'><input type=text name=email_form size=35 value='%s' class='form-control tpl_input_edicion_requerido'></div>",$email_form);
   $campos[] = $campo;

   $campo['nombre'] = _ROT_CONTACTO_DIRECCION;
   $campo['clase']  = "";
   $campo['input']  = sprintf("<div class='form-group'><input type=text name=direccion_form size=35 value='%s' class='form-control input1'></div>",$direccion_form);
   $campos[] = $campo;

   $campo['nombre'] = _ROT_CONTACTO_EMPRESA;
   $campo['clase']  = "";
   $campo['input']  = sprintf("<div class='form-group'><input type=text name=empresa_form size=35 value='%s' class='form-control input1'></div>",$empresa_form);
   $campos[] = $campo;

   $campo['nombre'] = _ROT_CONTACTO_COMENTA;
   $campo['clase']  = "requerido";
   $campo['input']  = sprintf("<div class='form-group'><textarea name=comenta_form rows=6 cols=40 align=center class='form-control tpl_input_edicion_requerido'>%s</textarea></div>",$comenta_form);
   $campos[] = $campo;
   ...
   return $smarty->fetch('tpl_contacto.html');
	...
	$body .= sprintf("\n\t\tCiudad: %s",$direccion_form);
	$body .= sprintf("\n\t\tActividad/Oficio: %s",$empresa_form);
	$body .= sprintf("\n\t\tComentario:\n\n%s",$comenta_form);
	...
	if (Funciones::microsmail($to,sprintf("Contacto desde la página %s",_NOMBRESITIO),$body,$from,'contacto.php','1') 
	  ...
	  $error[] = sprintf(_ROT_CONTACTO_ENVIO_CONFIRMA, _NOMBRESITIO);
	  $smarty1->assign('elementoMenu',$error);
